...
- **Oracle (IPO.GPT) — NEW MODULE — Phase 1 shipped**:
  ...
  - Free LLM stack: **Gemini 3 Flash** (general queries) + **Claude Sonnet 4.5** (DRHP analysis, fallback) — both via Emergent Universal LLM Key
  - **Free web search**: DuckDuckGo HTML scraper (no API key, no quota) with `source_filter` (all / official / news / academic)
  - **6-tier source grading** (🥇 SEBI/RBI/NSE/BSE → ⚪ web/community), inline numbered citations [1] [2], "References Used" collapsible block, 3 follow-up suggestions per response
  - **Long-term memory**: `oracle_memory` collection auto-extracts user-specific facts (sector, role, watchlist) via background `asyncio.create_task` and injects top 8 into system prompt on every turn
  - **DRHP PDF upload**: pypdf parsing → 60 chunks × ~2k chars stored in `oracle_drhp_chunks`; mode='drhp' answers with `[chunk-N]` citations
  - **Threads**: rename, pin, delete (cascades messages + chunks), grouped buckets (pinned / today / yesterday / last_7 / last_30 / older), search-by-title
  - **Bookmarks**: per-message bookmark collection
  - Frontend: `/oracle` page with collapsible sidebar + 3 tabs (General IPO Query / DRHP Deep Research / Application Queries) + bottom-anchored input + grid-line bg + Twitter-blue accent
  - Markdown rendering via `react-markdown` + `remark-gfm`; copy / print-PDF / bookmark actions per response
  - Floating **"Ask Oracle"** bot pill (top-right, every authenticated page, hidden on /oracle itself) mounted in `ProtectedRoute`
  - Sidebar entry **"Oracle {IPO.GPT}"** under workspace section with `Bot` icon
  - Persistent disclaimer footer: "Responses are generated by IPOLabs AI Pvt Ltd for informational purposes…"
  - **Tested**: 14/14 backend pytest pass; full frontend E2E passed (after fixing 2 issues — floating button mount + clipboard try/catch)
  - **MongoDB collections**: `oracle_threads`, `oracle_messages`, `oracle_memory`, `oracle_bookmarks`, `oracle_drhp_chunks`
  - Phase 2 deferred: true SSE streaming, embedded YouTube cards (needs YT Data API key), historical 30-year DRHP corpus indexing, full PWA install

### Changelog — Feb 6, 2026 (Valuation restore)
