...
BASE_URL = os.environ.get("REACT_APP_BACKEND_URL", "https://drhp-editor-suite.preview.emergentagent.com").rstrip("/")
API = f"{BASE_URL}/api"
...
class TestValuation:
    def test_projects_authenticated(self, session, user_token):
        r = session.get(f"{API}/bv-projects", headers=_auth(user_token), timeout=15)
        assert r.status_code == 200

    def test_projects_requires_auth(self, session):
        r = _fresh().get(f"{API}/bv-projects", timeout=15)
        assert r.status_code in (401, 403)
